...
void BatchVisualizer::dynamicReconfigureCallback(radiation_utils::batch_visualizerConfig& config, [[maybe_unused]] uint32_t level) {
  reconfigured = true;
...
/* addRay */  //{
void BatchVisualizer::addRay(Ray ray, double r, double g, double b, double a) {
...
/* addTriangle //{ */
void BatchVisualizer::addTriangle(Triangle tri, double r, double g, double b, double a, bool filled) {
...
/* addRectangle //{ */
void BatchVisualizer::addRectangle(Rectangle rect, double r, double g, double b, double a, bool filled) {
  std::vector<Triangle> triangles = rect.triangles();
...
/* addCuboid //{ */
void BatchVisualizer::addCuboid(Cuboid cuboid, double r, double g, double b, double a, bool filled) {

  for (int i = 0; i < 6; i++) {
    Rectangle             rect      = cuboid.getRectangle(i);
    std::vector<Triangle> triangles = rect.triangles();
    addTriangle(triangles[0], r, g, b, a, filled);
    addTriangle(triangles[1], r, g, b, a, filled);
  }
...
/* addEllipse //{ */
void BatchVisualizer::addEllipse(Ellipse ellipse, double r, double g, double b, double a, bool filled, int num_points) {
  std::vector<Eigen::Vector3d> points = buildEllipse(ellipse, num_points);

  if (filled) {
    for (int i = 0; i < num_points - 1; i++) {
      Triangle tri(ellipse.center(), points[i], points[i + 1]);
      addTriangle(tri, r, g, b, a, true);
    }
    Triangle tri(ellipse.center(), points[num_points - 1], points[0]);
    addTriangle(tri, r, g, b, a, true);

  } else {
    for (int i = 0; i < num_points - 1; i++) {
      Ray ray = Ray::twopointCast(points[i], points[i + 1]);
      addRay(ray, r, g, b, a);
    }
    Ray ray = Ray::twopointCast(points[num_points - 1], points[0]);
    addRay(ray, r, g, b, a);
  }
}
//}
...
/* addCylinder //{ */
void BatchVisualizer::addCylinder(Cylinder cylinder, double r, double g, double b, double a, bool filled, bool capped, int sides) {
  if (capped) {
    Ellipse top    = cylinder.getCap(Cylinder::TOP);
    Ellipse bottom = cylinder.getCap(Cylinder::BOTTOM);
    addEllipse(top, r, g, b, a, filled, sides);
    addEllipse(bottom, r, g, b, a, filled, sides);
  }
  std::vector<Eigen::Vector3d> top_points    = buildEllipse(cylinder.getCap(Cylinder::TOP), sides);
  std::vector<Eigen::Vector3d> bottom_points = buildEllipse(cylinder.getCap(Cylinder::BOTTOM), sides);
  for (unsigned int i = 0; i < top_points.size() - 1; i++) {
    Rectangle rect(bottom_points[i], bottom_points[i + 1], top_points[i + 1], top_points[i]);
    addRectangle(rect, r, g, b, a, filled);
  }
  Rectangle rect(bottom_points[bottom_points.size() - 1], bottom_points[0], top_points[0], top_points[top_points.size() - 1]);
  addRectangle(rect, r, g, b, a, filled);
}
//}

/* addCone //{ */
void BatchVisualizer::addCone(Cone cone, double r, double g, double b, double a, bool filled, bool capped, int sides) {
  if (capped) {
    Ellipse cap = cone.getCap();
  ...
  for (unsigned int i = 0; i < cap_points.size() - 1; i++) {
    Triangle tri(cap_points[i], cap_points[i + 1], cone.origin());
    addTriangle(tri, r, g, b, a, filled);
  }
  Triangle tri(cap_points[cap_points.size() - 1], cap_points[0], cone.origin());
  addTriangle(tri, r, g, b, a, filled);
}
//}

/* buildEllipse //{ */
std::vector<Eigen::Vector3d> BatchVisualizer::buildEllipse(Ellipse ellipse, int num_points) {
